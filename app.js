...
const EmailCampaignPage = () => {
  ...
  useEffect(() => {
    if (step === 0) {
      setLoadingList(true);
      api.get("/api/campaigns/email").then((res) => setCampaignsList(res.data.data || [])).catch((err) => alert("Failed to load campaigns")).finally(() => setLoadingList(false));
    }
  }, [step]);
...
const MassInvoicesPage = () => {
  ...
  useEffect(() => {
    if (step === 0) {
      setLoadingList(true);
      api.get("/api/invoices/batch").then((res) => setBatchList(res.data.data || [])).catch((err) => alert("Failed to load batch history")).finally(() => setLoadingList(false));
    }
  }, [step]);
  const [loading, setLoading] = useState(false);
  const [targetMode, setTargetMode] = useState("students");
  ...
  useEffect(() => {
    ...
    api.get("/api/students", { params: { col: "Id,name,code,photo,classroom,classroom_name", limit: 2e3, filters } }).then((res) => {
      const list = res.data?.data || res.data || [];
      list.forEach((s) => fetchedStudentsRef.current.set(s.Id || s.id, s));
      setStudents(list);
    }).catch(() => {
...
const SendRemindersPage = () => {
  ...
  useEffect(() => {
    if (step === 0) {
      setLoadingList(true);
      api.get("/api/students/reminders/batches").then((res) => setBatchList(res.data.data || [])).catch((err) => alert("Failed to load batch history")).finally(() => setLoadingList(false));
    }
  }, [step]);
  const [loading, setLoading] = useState(false);
  const [targetMode, setTargetMode] = useState("students");
